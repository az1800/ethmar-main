...
//     try {
//       const selectedCategory = categories.find((c) => c.id === activeFilter);
//       const response = await getPosts(selectedCategory?.supabaseCategory ?? null);
//       const allPostsData = Array.isArray(response.data) ? response.data : [];

//       if (!alive) return; // component unmounted — abort everything
...
//       const params = new URLSearchParams(searchParams.toString());
//       params.set("page", "1");
//       router.replace(`${pathname}?${params.toString()}`, { scroll: false }); // replace not push
//     } catch (error) {
//       console.error("Error fetching posts:", error);
//     } finally {
...
const categories = [
  { id: 1, name: "الجميع", supabaseCategory: null },
  { id: 2, name: "تحليل القطاعات", supabaseCategory: "تحليل القطاعات" },
  { id: 3, name: "البحوث المالية", supabaseCategory: "البحوث المالية" },
  { id: 4, name: "التحليل المالي", supabaseCategory: "التحليل المالي" },
  { id: 5, name: "قصة سهم", supabaseCategory: "قصة سهم" },
...
function PostsContent() {
  ...
  useEffect(() => {
    // Guard: if the component unmounts while the fetch is in flight,
    // do NOT call router.replace or set state — that's what was causing
    // the redirect back to the blog page mid-navigation.
    let alive = true;
    ...
    async function fetchAllPosts() {
      ...
      try {
        ...
        // Use replace (not push) so page resets don't add to browser history
        const params = new URLSearchParams(searchParams.toString());
        params.set("page", "1");
  ...
  // Paginate client-side whenever the page or post list changes
  useEffect(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
